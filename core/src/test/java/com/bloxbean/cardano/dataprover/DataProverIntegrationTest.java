...
import com.bloxbean.cardano.dataprover.test.TestDataProvider;
...
class DataProverIntegrationTest {
    ...
    @Autowired
    private MerkleRegistry trieRegistry;
    ...
    void testCreateTrie() {
        ...
        ResponseEntity<MerkleResponse> response = restTemplate.postForEntity(
                API_BASE + "/merkle",
                request,
                MerkleResponse.class
        );
    ...
    void testGetTrie() {
        ...
        ResponseEntity<MerkleResponse> response = restTemplate.getForEntity(
                API_BASE + "/merkle/" + trieId,
                MerkleResponse.class
        );
    ...
    void testIngestData() {
        ...
        assertThat(response.getBody()).isNotNull();
        assertThat(response.getBody().getMerkleIdentifier()).isEqualTo(trieId);
    ...
    void testVerifyInclusionProof() {
        ...
        ResponseEntity<ProofVerificationResponse> verifyResponse = restTemplate.postForEntity(
                API_BASE + "/merkle/" + trieId + "/proofs/verify",
                verifyRequest,
    ...
    void testBatchProofs() {
        ...
        ResponseEntity<List<ProofGenerationResponse>> response = restTemplate.exchange(
                API_BASE + "/merkle/" + trieId + "/proofs/batch",
                HttpMethod.POST,
    ...
    void testGetRootHash() {
        ...
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(response.getBody()).isNotNull();
        assertThat(response.getBody()).containsKey("merkleIdentifier");
        assertThat(response.getBody()).containsKey("rootHash");
        assertThat(response.getBody().get("merkleIdentifier")).isEqualTo(trieId);
    ...
    void testDeleteTrie() {
        ...
        // Delete the trie
        restTemplate.delete(API_BASE + "/merkle/" + trieId);
    ...
    void testTrieNotFound() {
        ResponseEntity<ErrorResponse> response = restTemplate.getForEntity(
                API_BASE + "/merkle/non-existing-trie-12345",
                ErrorResponse.class
        );
    ...
    void testAddEntriesDirectly() {
        ...
        ResponseEntity<AddEntriesResponse> response = restTemplate.postForEntity(
                API_BASE + "/merkle/" + trieId + "/entries",
                request,
